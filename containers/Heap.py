...
class Heap(BinaryTree):
    ...
    def insert(self, value):
        ...
        of the total number of nodes
            ...
            runtime
            1. See https://stackoverflow.com/questions/18241192/impleme
nt-heap-using-a-binary-tree for hints
        ...
        if self.root:
            count = self.__len__()
            line = "{0:b}".format(count + 1)[1:]
            self.root = Heap._insert(self.root, value, line)
        else:
    ...
    def _remove_bottom_right(node, remove):
        ...
        if remove[0] == '0':
            if len(remove) == 1:
                ...
                removed, node.left = Heap._remove_bottom_right(
                    node.left, remove[1:])
        if remove[0] == '1':
            if len(remove) == 1:
                ...
                removed, node.right = Heap._remove_bottom_right(
                    node.right, remove[1:])
